...
$(function() {
    ...
    $(".dropdown-menu li a").click(function() {
        ...
        $(this).parents(".dropdown").find('.btn').val($(this).data(
            'value'));
        console.log($(this).attr('data-ref') + ' SELECTED.');
        // load the data
        loadData($(this).attr('data-ref'), 'pred', ['blendid',
            'corpid', 'datetime'
        ]);
    });
    ...
    function loadData(filepath, yvar, nonplots) {
        ...
        $.ajax({
            ...
            $.each(data, function(index, item) {
                $.each(Object.keys(item), function(i,
                    key) {
                    // skip the response variable
                    if (key === yvar) return;
                    if (Object.keys(xvals).indexOf(
                            key) < 0) {
                        xvals[key] = [];
                    }
                    if (xvals[key].indexOf(item[
                            key]) < 0) {
                        xvals[key].push(item[
                            key]);
                    }
            ...
            chartArray = createHtmlTable(data, xvals, gridObj,
                xNames[0],
                selector);
    ...
    function initializeButtons(data, xvals, gridvars, plotvar, selector) {
        ...
        $('label[id^="label-"]').on('click', function() {
            ...
            createHtmlTable(data, xvals, gridObj, plotvar,
                selector);
        });
    ...
    function createHtmlTable(data, xvals, gridvars, plotvar, selector) {
        ...
        if (rowvar === undefined) {
            ...
            chartArray = createSingleRow(data, xvals, colvar, plotvar,
                selector);
        } else if (colvar === undefined) {
            // console.log('SHOULD CREATE ONE COLUMN');
            chartArray = createSingleCol(data, xvals, rowvar, plotvar,
                selector);
        } else {
            chartArray = createFullTable(data, xvals, rowvar, colvar, plotvar,
                selector);
        }
...
function setChartSize(chart) {
    ...
    chart.setSize($(selector).width(), $(selector).height());
    return false;
}
...
Highcharts.getSVG = function(charts, numRows, numCols, options, callback) {
    var svgArr = [],
        ...
        svgResult = function(svgres) {
            ...
            var col = i % numCols;
            var svg = svgres.replace('<svg', '<g transform="translate(' +
                width + ',' + top + ')" ');
            ...
            if (svgArr.length === charts.length) {
                top += 20;
                width += 60;
                var svgAll = '<svg height="' + top + '" width="' + width +
...
Highcharts.exportCharts = function(charts, options) {
    ...
    Highcharts.getSVG(charts, charts.numRows, charts.numCols, options,
        function(svg) {
            Highcharts.downloadSVGLocal(svg, (options.filename ||
                    'chart') + '.' + (imageType === 'image/svg+xml' ?
...
function pushCellDataToChart(cellData, xvals, rowvar, row, colvar, col, plotvar) {
    ...
    var options = {
        ...
        tooltip: {
            ...
            formatter: function() {
                // var str = '<b>' + plotvar + ': ' + this.x + '</b>';
                var str = '<h4>' + plotvar + ': ' + this.x +
                    '</h4><table class="table table-condensed" id="tooltip-table"><tr><th>Blend ID</th><th>Prediction</th></tr>';

                var pts = this.points.sort(dynamicSort('-y'));
                $.each(pts, function(index, point) {
                    // str += '<br/><b>' + point.series.name + ':</b> ' + point.y;
                    str += '<tr><td style="color: ' + point.series
                        .color + '"><b>' + point.series.name +
                        '</b></td><td>' + point.y +
                        '</td></tr>';
                });
...
function createSingleRow(data, xvals, colvar, plotvar, selector) {
    ...
    columns.map(function(col) {
        ...
        var c = pushCellDataToChart(cellData, xvals, '', '', colvar,
            col, plotvar);
        chartArray.push(c);
    });
...
function createSingleCol(data, xvals, rowvar, plotvar, selector) {
    ...
    rows.map(function(row) {
        ...
        var c = pushCellDataToChart(cellData, xvals, rowvar, row, '',
            '', plotvar);
        chartArray.push(c);
    });
...
function createFullTable(data, xvals, rowvar, colvar, plotvar, selector) {
    ...
    rows.map(function(row) {
        columns.map(function(col) {
            var cellData = pullCellData(data, rowvar, row,
                colvar, col);
            var c = pushCellDataToChart(cellData, xvals, rowvar,
                row, colvar, col, plotvar);
            chartArray.push(c);
        });
    });
